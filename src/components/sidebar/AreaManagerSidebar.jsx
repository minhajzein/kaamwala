...
const SidebarData = [
	...
	},
	{
		name: 'Restaurant',
		icon: IoRestaurant,
		path: '/area-manager/restaurant',
		color: 'bg-red-500',
	},
	{
		name: 'employee',
		icon: IoMan,
		path: '/area-manager/employee',
		color: 'bg-green-500',
	},
	{
		name: 'Settings',
		icon: IoSettings,
		path: '/area-manager/settings',
		color: 'bg-violet-500',
	},
]
				...
				<div className='flex-grow mt-2  no-scrollbar'>
					{SidebarData.map((data, i) => {
						...
						const itemColor = isActive(data.path)
							...
							: 'hover:bg-gray-100 text-gray-800 rounded-md'
						const iconColor = isActive(data.path) ? 'text-gray-900' : ''

						return (
							<div
								key={i}
								className={`  transition duration-300 ease-in-out mx-2 py-[2px]`}
							>
								<div
								...
								>
									...
									>
										...
										{isOpen && (
											<div className={` ${iconColor}`}>{data.name}</div>
										)}
									</div>
								</div>
							</div>
