...
const JobExperienceTimeline = () => {
  const [expanded, setExpanded] = useState(new Array(jobExperiences.length).fill(false));
  ...
  return (
    <Timeline mode="left" className="space-y-6">
      {jobExperiences.map((job, index) => (
        <Timeline.Item key={index} className="fade-in-up">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
                    ...
                    <div className="space-y-1 text-xs md:text-sm">
                      <div className="flex items-center">
                        ...
                        <Rate className="text-xs md:text-base" allowHalf disabled defaultValue={job.hygiene} />
                      </div>
                      <div className="flex items-center">
                        <strong className="w-1/2">Wastage Control:</strong>
                        <Rate className="text-xs md:text-base" allowHalf disabled defaultValue={job.wastageControl} />
                      </div>
                      <div className="flex items-center">
                        <strong className="w-1/2">Communication:</strong>
                        <Rate className="text-xs md:text-base" allowHalf disabled defaultValue={job.communication} />
                      </div>
                      <div className="flex items-center">
                        <strong className="w-1/2">Attendance:</strong>
                        <Rate className="text-xs md:text-base" allowHalf disabled defaultValue={job.attendance} />
                      </div>
                      <div className="flex items-center">
                        <strong className="w-1/2">Productivity:</strong>
                        <Rate className="text-xs md:text-base" allowHalf disabled defaultValue={job.productivity} />
                      </div>
                    </div>
                  </div>
                )}
