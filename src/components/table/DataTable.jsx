...
import { FaChevronDown, FaFileDownload, FaFileExcel, FaFilePdf, FaTimes } from 'react-icons/fa';
...
const DataTable = ({ data, columns, filterColumn,filterColumn2, title, total, Export,type,nopagination,navigate }) => {
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter((item) => {
      ...
      const filter2Match =
        ...
        (filterValue2 === '' ||
          item?.[filterColumn2]?.toString().toLowerCase().includes(filterValue2.toLowerCase()));
  ...
  const renderTableRows = () => {
    if (filteredData.length === 0) {
      return (
        <tr className="bg-beige-200">
          <td colSpan={columns.length} className="md:py-4 py-2 px-2 text-start text-gray-600">
            No data available
          </td>
        </tr>
      );
    ...
    const emptyRows = Array.from({ length: itemsPerPage - page.length }).map((_, index) => (
      <tr key={`empty-${index}`} className="bg-beige-200">
        {columns.map((column, colIndex) => (
          <td key={colIndex} className="md:py-2 py-2 px-2 text-start text-gray-600">
            &nbsp;
          </td>
        ))}
      </tr>
    ...
    return (
      <>
        {page.map((row) => {
          prepareRow(row);
          return (
            <tr {...row.getRowProps()} className="bg-beige-100 text-gray-800" key={row.id}>
              {row.cells.map((cell) => (
                <td {...cell.getCellProps()} className="md:py-2 text-[.8rem] border-b py-2 px-2 whitespace-nowrap">
                  {cell.render('Cell')}
                </td>
              ))}
            </tr>
    ...
    <div className="container bg-white  border rounded-xl border-dotted no-scrollbar  p-2 md:pt-4 md:px-4 md:pb-2 min-w-full bg-beige-50">
      <div className="flex justify-between items-center md:mb-4 mb-2">
        <div className="flex md:gap-3 gap-1">
          ...
          <div className={`${filterColumn2 === undefined && 'hidden'} relative flex items-center`}>
            <select
              value={filterColumn2}
              onChange={(e) => setFilterValue(e.target.value)}
              className="bg-beige-200 appearance-none md:px-3 w-full px-1  py-1 md:pr-8 border text-gray-800 border-gray-500 rounded-lg focus:ring-blue-500 focus:border-blue-500 text-xs md:text-sm"
            >
              ...
              {[...new Set(data?.map((item) => item?.[filterColumn2]))].map((value, index) => (
                <option key={index} value={value}>
                  {value}
                </option>
              ))}
  ...
  <div className="relative flex items-center" ref={dropdownRef}>
    ...
    >
      <span>{isOpen ? <IoIosCloseCircleOutline  className='h-4 w-4 focus-within::animate-spin' /> : <MdOutlineDownloading   className='h-4 w-4 ' />}</span>
    ...
    {isOpen && (
      <div className="absolute top-12 right-0 bg-white border border-gray-300 rounded-lg shadow-lg mt-2 w-48 ">
        <button
          className="block w-full text-left px-4 py-2 text-gray-800 hover:bg-gray-100 flex items-center space-x-2 transition-colors"
          onClick={generatePDF}
        >
          <FaFilePdf />
