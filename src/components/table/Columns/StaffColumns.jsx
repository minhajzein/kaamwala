...
export const HiringStatus = ({ hiringStatus }) => {
  const statusClass = hiringStatus == 'available' ? 'bg-green-50 border-green-400 border text-green-700' : hiringStatus == 'notavailable'? 'bg-orange-50 border border-orange-400 text-orange-700' : hiringStatus == 'blacklisted' ? 'bg-red-50 border border-red-400 text-red-700':'';
  return (
...
const StaffColumns = (viewActionClick, editActionClick) => [
        ...
        { Header: 'Hiring Status', accessor: 'hiringStatus',
          Cell: ({ row }) => (
            <HiringStatus hiringStatus={row.original.hiringStatus} />
          ),
         },
         {
          Header: () => (
